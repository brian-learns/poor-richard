...
* [lunardate](https://github.com/lidaobing/python-lunardate) *(candidate — not installed)*: Pure-Python Chinese (lunisolar) calendar — Gregorian ↔ lunar year/month/day with leap-month handling and zodiac, e.g. `LunarDate.fromSolarDate(2024, 2, 10)`. The maintained alternative for Chinese-calendar conversion after convertdate 2.x dropped its `chinese` module. `uv add lunardate` — **GPL-3.0**: excluded from the default install per the curation rules.
