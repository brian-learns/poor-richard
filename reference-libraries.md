...
* [Babel](https://github.com/python-babel/babel): Locale-level reference data from CLDR (Unicode Common Locale Data Repository) — locale display names, plural rules, and number/currency/date formatting per locale. Complements pycountry/iso639 (code registries) at the locale level. Gotchas (2.18): plural evaluation is `Locale('ru').plural_form(n)` (property returning a callable, old `plural_rule()` gone), and `get_display_name()` is `None` for most region-specific locales — use the base locale. `uv add Babel` (33 MB installed; 32 MB is bundled CLDR data)
...
* [h3](https://github.com/uber/h3-py): Uber's H3 hexagonal geospatial indexing — convert lat/lng ↔ hierarchical hex cell indices (resolutions 0–15), with grid distances, rings, and cell geometry, fully offline (bundled C extension). Gotchas: H3 v4 cell IDs are incompatible with v3 (old-docs indices raise `H3CellInvalidError`), and the v4 API is flat (`latlng_to_cell`, `cell_to_latlng`). `uv add h3`
* [shapely](https://github.com/shapely/shapely): The industry-standard Python interface to GEOS — manipulation and analysis of planar geometric features (points, lines, polygons): distance, area, centroid, buffers, set operations, WKT/GeoJSON I/O. Pure computation, no data; the wheel bundles GEOS (`shapely.libs/`), so it is fully offline. `uv add shapely` — note 2.x exposes vectorized top-level functions (`shapely.area`, `shapely.centroid`); the `shapely.geos` module is deprecated.
* [pyproj](https://github.com/pyproj4/pyproj): Python interface to PROJ — cartographic projections and coordinate transforms (Web Mercator, UTM, geodetic ↔ geocentric) plus Karney geodesics (`Geod`), over a bundled EPSG registry snapshot (proj-data 1.24 / EPSG v12.029). Pairs with geographiclib: `Geod(ellps='WGS84')` agrees with `Geodesic.WGS84` to full double precision. Gotchas: geocentric is **EPSG:4328**, not 4979 (that one is WGS 84 geodetic 3D — 4326→4979 silently selects a null offset), use 4979→4328 for full ECEF, pass `always_xy=True`, and UTM zone n's central meridian is 6n−183 (zone 31N: +3). `uv add pyproj`
...
* [convertdate](https://github.com/fitnr/convertdate): Precise, offline calendar conversions between the Gregorian calendar and the Julian, Hebrew, Islamic (tabular), French Republican, Persian, and Mayan calendars — everything routed through Julian Day arithmetic, e.g. `convertdate.hebrew.from_gregorian(2024, 10, 3)` → `(5785, 7, 1)` (Rosh Hashanah 5785). `uv add convertdate` — note the 2.x API is per-module `from_gregorian`/`to_gregorian` (1.x's `gregorian_to_<cal>` functions are gone) and the 1.x Chinese-calendar module was removed.
* [lunardate](https://github.com/lidaobing/python-lunardate): Pure-Python Chinese (lunisolar) calendar — Gregorian ↔ lunar year/month/day with leap-month handling, e.g. `LunarDate.from_solar_date(2024, 2, 10)` → `LunarDate(2024, 1, 1)` (Chinese New Year 2024). The maintained alternative for Chinese-calendar conversion after convertdate 2.x dropped its `chinese` module. `uv add lunardate` — **GPL-3.0**: installed by explicit maintainer decision (opt-in). Gotchas: the 0.3.0 API is snake_case (`from_solar_date`/`to_solar_date`/`leap_month_for_year`; the camelCase forms are deprecated) and the bundled table covers 1900–2099 only.
...
* [icalendar](https://github.com/collective/icalendar): RFC 5545 (iCalendar) parser and generator — `.ics` content lines, VEVENT/VTIMEZONE/VALARM components, DATE-TIME & DURATION property values, plus RFC 7265 (jCal) JSON via `to_jcal()`/`from_jcal()`. `uv add icalendar` — note `Calendar.from_ical()` on a bare VEVENT returns an `Event` (not a Calendar), and `to_ical()` yields CRLF-terminated bytes.
...
* [Unidecode](https://pypi.org/project/Unidecode/): ASCII transliteration of Unicode text from a bundled, language-blind table — `unidecode.unidecode('café')` → `'cafe'`, `'straße'` → `'strasse'`, `'Ω'` → `'O'` (single-letter Greek). Gotchas: CJK maps to pinyin regardless of language (北京 → `'Bei Jing '`, with a trailing space), and this is NOT `text-unidecode` (a different package). **GPL-2.0-or-later** — accepted for this project on maintainer decision. `uv add Unidecode`
...
* [pymeeus](https://github.com/BillGray/PyMeeus): Jean Meeus's *Astronomical Algorithms* (2nd ed.) as a pure-Python reference — sun/moon geocentric positions, obliquity, nutation, aberration, equinox/solstice finder, lunar libration. Serves as the independent oracle for the ephem moon-phase xfail canary (agrees with pysweph to <0.01°). Gotcha: module and class share names (`pymeeus.Sun.Sun`, `pymeeus.Epoch.Epoch`). `uv add pymeeus` (LGPL-3.0; sdist only)
